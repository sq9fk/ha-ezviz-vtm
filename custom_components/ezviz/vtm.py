...
class EzvizVtmData:
    ...
    # serial -> EzvizClient of the account the camera belongs to
...
def vtm_stream_url(hass: HomeAssistant, serial: str) -> str:
...
def rtsp_available(camera_data: dict[str, Any]) -> bool:
...
class EzvizVtmStreamView(HomeAssistantView):
    ...
    async def get(
        ...
        pump.start()
        feeder = asyncio.create_task(_feed_ffmpeg(queue, process.stdin))
